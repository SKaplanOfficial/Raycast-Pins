 ...
 * @param setPins The function to update the list of pins.
 */
export const deletePin = async (pin: Pin, setPins: React.Dispatch<React.SetStateAction<Pin[]>>, showAlert = true, displayToast = true) => {
  if (!showAlert || (await confirmAlert({ title: `Delete Pin '${pin.name}'`, message: "Are you sure?", primaryAction: { title: "Delete", style: Alert.ActionStyle.Destructive } }))) {
    const storedPins = await getStorage(StorageKey.LOCAL_PINS);
 ...
 */
export const sortPins = (pins: Pin[], groups: Group[], sortMethod?: keyof typeof SORT_STRATEGY, sortFn?: (a: unknown, b: unknown) => number) => {
...
export const getPinStatistics = (pin: Pin, pins: Pin[], format: "string" | "object" = "string") => {
  ...
  const percentOfAllExecutionsText = `${percentOfAllExecutions == "NaN%" ? "0%" : percentOfAllExecutions} of All Pin Executions`;
